...
"Modulo con funciones auxiliares para el manejo de errores y temas comunes"
...
def exception_info(current_filename=None, index=-1):
    ...
    # guardo el traceback original (por si hay una excepcion):
    info = sys.exc_info() #         exc_type, exc_value, exc_traceback
    # importante: no usar unpacking porque puede causar memory leak
    if not current_filename:
        # genero un call stack para ver quien me llamo y limitar la traza: 
    ...
    # extraer la ultima traza del archivo solicitado:
    # (util para no alargar demasiado la traza con lineas de las librerias)
...
def inicializar_y_capturar_excepciones(func):
    ...
    def capturar_errores_wrapper(self, *args, **kwargs):
        try:
            ...
            self.params_in = {}
            self.params_out = {}
            # llamo a la funcion (con reintentos)
            ...
            while retry:
                try:
                    ...
                    if e[0] not in (10054, 10053):
                        # solo reintentar si el error es de conexion
            ...
            # guardo destalle de la excepcion SOAP
...
def inicializar_y_capturar_excepciones_simple(func):
    "Decorador para inicializar y capturar errores (version basica indep.)"
    @functools.wraps(func)
    def capturar_errores_wrapper(self, *args, **kwargs):
        self.inicializar()
        try:
...
class BaseWS:
    ...
    def Conectar(self, cache=None, wsdl=None, proxy="", wrapper=None, cacert=None, timeout=30, soap_server=None):
        ...
        try:
            ...
            if self.HOMO or not wsdl:
                wsdl = self.WSDL
            # agregar sufijo para descargar descripcion del servicio ?WSDL o ?wsdl
            if not wsdl.endswith(self.WSDL[-5:]) and wsdl.startswith("http"):
                wsdl += self.WSDL[-5:]
            if not cache or self.HOMO:
                # use 'cache' from installation base directory 
                cache = os.path.join(self.InstallDir, 'cache')
            # deshabilitar verificacion cert. servidor si es nulo falso vacio
            if not cacert:
                cacert = None
            elif cacert is True:
            ...
            # corrijo ubicacion del servidor (puerto http 80 en el WSDL AFIP)
    ...
    def log(self, msg):
        "Dejar mensaje en bitacora de depuracion (metodo interno)"
    ...
    def DebugLog(self):
        "Devolver y limpiar la bitacora de depuracion"
        if self.Log:
    ...
    def LoadTestXML(self, xml):
        "Cargar un archivo de pruebas con la respuesta simulada (depuracion)"
    ...
    def ObtenerTagXml(self, *tags):
        ...
        try:
            if self.xml:
                xml = self.xml
                # por cada tag, lo busco segun su nombre o posicion
                for tag in tags:
    ...
    def SetParametros(self, cuit, token, sign):
        "Establece un parametro general"
        self.Token = token
        self.Sign = sign
        self.Cuit = cuit
        return True
    ...
    def SetParametro(self, clave, valor):
        "Establece un parametro de entrada (a usarse en llamada posterior)"
        # util para parametros de entrada (por ej. VFP9 no soporta mas de 27)
        self.params_in[str(clave)] = valor
        return True

    def GetParametro(self, clave, clave1=None, clave2=None, clave3=None, clave4=None):
        "Devuelve un parametro de salida (establecido por llamada anterior)"
        # util para parametros de salida (por ej. campos de TransaccionPlainWS)
        ...
        for d in l:
            ...
            if agrega or not tabla:
                ...
                if not tabla.eof() and len(l) > 1:
                    if DEBUG: print "Moviendo al proximo registro ", tabla.record_number
                    tabla.next()
        tabla.close()
...
def dar_nombre_campo_dbf(clave, claves):
    ...
    # si el campo esta repetido, le agrego un numero
    i = 0
...
def verifica(ver_list, res_dict, difs):
    ...
    for k, v in ver_list.items():
        ...
        if isinstance(v, list):
            ...
            # tipos diferentes, comparo la representacion  
...
def abrir_conf(config_file, debug=False):
    "Abrir el archivo de configuracion (usar primer parametro como ruta)"
    # en principio, usar el nombre de archivo predeterminado
    # si se pasa el archivo de configuracion por parametro, confirmar que exista
    # y descartar que sea una opcion
    if len(sys.argv)>1:
